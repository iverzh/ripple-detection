...
NSx.FID                       = fopen([path fname], 'r', 'ieee-le');
NSx.MetaTags.Filename     = fname;
NSx.MetaTags.FilePath     = path(1:end-1);
NSx.MetaTags.FileExt      = fext;
NSx.MetaTags.FileTypeID   = fread(NSx.FID, [1,8]   , '*char');
if strcmpi(NSx.MetaTags.FileTypeID, 'NEURALSG')
	NSx.MetaTags.FileSpec      = '2.1';
    NSx.MetaTags.SamplingLabel = fread(NSx.FID, [1,16]  , '*char');
    NSx.MetaTags.TimeRes       = 30000;
    NSx.MetaTags.SamplingFreq  = NSx.MetaTags.TimeRes / fread(NSx.FID, 1 , 'uint32=>double');
    ChannelCount               = double(fread(NSx.FID, 1       , 'uint32=>double'));
    NSx.MetaTags.ChannelCount  = ChannelCount;
    NSx.MetaTags.ChannelID     = fread(NSx.FID, [ChannelCount 1], '*uint32');
    try
    	t                          = dir([path fname]);
    	NSx.MetaTags.DateTime      = t.date;
    end
elseif strcmpi(NSx.MetaTags.FileTypeID, 'NEURALCD')
    BasicHeader                = fread(NSx.FID, 306, '*uint8');
    ...
    ExtendedHeader             = fread(NSx.FID, readSize, '*uint8');
 ...
 %  NSx.ElectrodesInfo(headerIDX).ChannelResolution = ((double(NSx.ElectrodesInfo.MaxAnalogValue) - double(NSx.ElectrodesInfo.MinAnalogValue)) /(double(NSx.ElectrodesInfo.MaxDigiValue) - double(NSx.ElectrodesInfo.MinDigiValue)));
...
f.EOexH = double(ftell(NSx.FID));
fseek(NSx.FID, 0, 'eof');
f.EOF = double(ftell(NSx.FID));
% Read Raw Header for saveNSx
fseek(NSx.FID, 0, 'bof');
NSx.RawData.Headers = fread(NSx.FID, f.EOexH, '*uint8');
NSx.RawData.DataHeader = fread(NSx.FID, 9, '*uint8');
fseek(NSx.FID, f.EOexH, 'bof');
...
%    while double(ftell(NSx.FID)) < f.EOF
%        headerCount = headerCount + 1;
%        DataHeader{headerCount} = fread(NSx.FID, 9);
%        fseek(NSx.FID,-9,'cof');
%        fread(NSx.FID,5);
%        DataPoints(headerCount) = fread(NSx.FID,1,'uint32');


%        f.BOData(headerCount) = double(ftell(NSx.FID));
%        fseek(NSx.FID, DataPoints(headerCount) * ChannelCount * 2, 'cof');
%        f.EOData(headerCount) = double(ftell(NSx.FID));
%    end
...
elseif strcmpi(NSx.MetaTags.FileTypeID, 'NEURALCD')    
    ...
    while double(ftell(NSx.FID)) < f.EOF
        ...
        NSx.MetaTags.Timestamp(segmentCount)  = fread(NSx.FID, 1, 'uint32');
        NSx.MetaTags.DataPoints(segmentCount) = fread(NSx.FID, 1, 'uint32');
        f.BOData(segmentCount) = double(ftell(NSx.FID));
        fseek(NSx.FID, NSx.MetaTags.DataPoints(segmentCount) * ChannelCount * 2, 'cof');
        f.EOData(segmentCount) = double(ftell(NSx.FID));
        ...
        NSx.MetaTags.DataPoints(segmentCount) = (f.EOData(segmentCount)-f.BOData(segmentCount))/(ChannelCount*2);
         %disp(NSx.MetaTags.DataPoints(segmentCount));
    end
end
...
if length(NSx.MetaTags.DataPoints) > 1
    ...
    if modifiedTime == 1
        ...
        fclose(NSx.FID); clear variables; if nargout; varargout{1} = -1; end; return;
    end
end
...
if strcmp(ReadData, 'read')
    ...
    if NSx.RawData.PausedFile
        for dataIDX = 1:segmentCount
            fseek(NSx.FID, f.BOData(dataIDX), 'bof');
            % Skip the file to the first channel to read
            fseek(NSx.FID, (find(NSx.MetaTags.ChannelID == min(userRequestedChannels))-1) * 2, 'cof');        
            NSx.Data{dataIDX} = fread(NSx.FID, [numChansToRead NSx.MetaTags.DataPoints(dataIDX)], [num2str(numChansToRead) precisionType], double((ChannelCount-numChansToRead)*2 + ChannelCount*(skipFactor-1)*2));
        end    
    else
        fseek(NSx.FID, f.BOData, 'bof');
        % Skip the file to the beginning of the time requsted, if not 0
        fseek(NSx.FID, (StartPacket - 1) * 2 * ChannelCount, 'cof');
        
        % Skip the file to the first channel to read
        fseek(NSx.FID, (find(NSx.MetaTags.ChannelID == min(userRequestedChannels))-1) * 2, 'cof');        
        NSx.Data = fread(NSx.FID, [numChansToRead DataLength], [num2str(numChansToRead) precisionType], double((ChannelCount-numChansToRead)*2 + ChannelCount*(skipFactor-1)*2));
    end
end
